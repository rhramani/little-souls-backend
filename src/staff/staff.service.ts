...
export class StaffService {
  ...
  async findAllStaff(page = 1, limit = 20) {
    ...
    const [staff, total, activeCount, disabledCount] = await this.prisma.$transaction([
      this.prisma.staffProfile.findMany({
        ...
        include: {
          users: {
            select: { id: true, email: true, isActive: true, userType: true, lastLoginAt: true },
          },
        },
      }),
  ...
  async findOneStaff(staffId: string) {
    const staff = await this.prisma.staffProfile.findUnique({
      ...
      include: {
        users: {
          select: { id: true, email: true, mobile: true, isActive: true, userType: true, lastLoginAt: true },
        },
      },
    });
  ...
  async deactivateStaff(staffId: string) {
    ...
    return { message: `Staff '${staffId}' and linked user accounts deactivated.` };
  }
  ...
  async assignCustomer(dto: AssignCustomerDto) {
    const staffUser = await this.prisma.user.findUnique({ where: { id: dto.salesStaffId } });
    if (!staffUser) throw new NotFoundException(`Staff user '${dto.salesStaffId}' not found.`);
  ...
  async getStaffPerformance(salesStaffId: string) {
    ...
    if (!staffUser) throw new NotFoundException(`Staff user '${salesStaffId}' not found.`);
    if (staffUser.userType !== UserType.STAFF && staffUser.userType !== UserType.SUPER_ADMIN) {
    ...
    const salesOrders = await this.prisma.order.findMany({
      where: { handledBySalesStaffId: salesStaffId, orderStatus: { not: 'CANCELLED' } },
      select: { id: true, grandTotal: true },
    });

    const totalOrdersCount = salesOrders.length;
    let totalSalesVolume = new Prisma.Decimal(0);
    for (const order of salesOrders) totalSalesVolume = totalSalesVolume.add(order.grandTotal);
  ...
  async getStaffLeaderboard() {
    const staffUsers = await this.prisma.user.findMany({
      where: { userType: { in: [UserType.STAFF, UserType.SUPER_ADMIN] }, isActive: true },
      select: { id: true },
    });
  ...
  async getAttendanceHistory(staffId?: string, startDate?: string, endDate?: string, page = 1, limit = 30) {
    const skip = (page - 1) * limit;
    const where: any = {};
    ...
    return { records, meta: { total, page, limit, totalPages: Math.ceil(total / limit) } };
  }
  ...
  async markAttendanceAdmin(dto: MarkAttendanceDto, approvedById: string) {
    ...
    return this.prisma.attendanceRecord.upsert({
      where: { staffId_attendanceDate: { staffId: dto.staffId, attendanceDate } },
  ...
  async checkInAttendance(userId: string, note?: string) {
    const user = await this.prisma.user.findUnique({ where: { id: userId }, select: { staffId: true } });
    if (!user?.staffId) throw new BadRequestException('User is not linked to a staff profile.');

    const today = new Date();
    today.setHours(0, 0, 0, 0);

    const existing = await this.prisma.attendanceRecord.findUnique({
      where: { staffId_attendanceDate: { staffId: user.staffId, attendanceDate: today } },
    });
    ...
    if (existing) {
      return this.prisma.attendanceRecord.update({
        ...
        data: { checkInTime: new Date(), status: 'PRESENT', note: note || existing.note },
      });
    }

    return this.prisma.attendanceRecord.create({
      data: { staffId: user.staffId, attendanceDate: today, status: 'PRESENT', checkInTime: new Date(), note },
    });
  }
  ...
  async checkOutAttendance(userId: string) {
    ...
    if (!existing?.checkInTime) throw new BadRequestException('Must check in before checking out.');
    if (existing.checkOutTime) throw new BadRequestException('Already checked out today.');

    const checkOutTime = new Date();
    const totalWorkMinutes = Math.floor((checkOutTime.getTime() - existing.checkInTime.getTime()) / 60000);
  ...
  async getLeaveRequests(userId: string, userType: string, page = 1, limit = 20) {
    const skip = (page - 1) * limit;
    const where: any = {};
    ...
    return { requests, meta: { total, page, limit, totalPages: Math.ceil(total / limit) } };
  }
  ...
  async approveLeave(leaveId: string, userId: string) {
    ...
    if (leave.status !== 'PENDING') throw new BadRequestException(`Leave is already ${leave.status}.`);
  ...
  async getPayrolls(staffId?: string, month?: number, year?: number, page = 1, limit = 20) {
    const skip = (page - 1) * limit;
    const where: any = {};
    if (staffId) where.staffId = staffId;
    ...
    return { payrolls, meta: { total, page, limit, totalPages: Math.ceil(total / limit) } };
  }
  ...
  async markPayrollPaid(payrollId: string, userId: string) {
    ...
    if (payroll.paymentStatus === 'PAID') throw new BadRequestException('Payroll already marked as paid.');
  ...
  async calculatePayroll(staffId: string, salaryMonth: string | number, salaryYear: string | number, userId: string) {
    const staff = await this.prisma.staffProfile.findUnique({ where: { id: staffId } });
    if (!staff) throw new NotFoundException(`Staff Profile '${staffId}' not found.`);
